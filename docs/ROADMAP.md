...
| **1. Investigation Workspace** | 🟢 DONE (P0) | Fast investigation search, Sources, Evidence, Claims, Timeline, Gaps | **P0 shipped 2026-08-28** — the full SEARCH → FIND RECEIPT → SAVE EVIDENCE → FINDING → REPORT loop works end-to-end, verified live: search returns the actual matched passage + PDF page number, "Save as Evidence" auto-carries excerpt/location/provenance, Claims link evidence, a deterministic Evidence-Backed Report generates from Claims. AI Qualitative Analysis shipped too (Anthropic Claude Haiku, provider-agnostic router, opt-in/paid per CANON.md AI section) alongside always-free deterministic Quantitative Analysis. The 9-tab workspace was simplified to 4 (Research / Evidence / Findings / Report) the same day for first-time-user usability — see `docs/CLAUDE_HANDOFF.md`. Bibliography built too. Outstanding within this phase: Contradiction detection is currently AI-only (Qualitative Analysis' `contradictions` field) — a deterministic version (two Claims that can't both be true) is Phase 4 below. |
